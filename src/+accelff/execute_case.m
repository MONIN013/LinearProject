...
assert(string(job.method)=="fb" || string(job.method)=="thesis" || ...
    string(job.method)=="acceleration",'accelff:Method','Unknown comparison method.');
...
if string(job.method)=="fb"
    assert(all(job.f==0),'accelff:Baseline','The FB-only condition must have zero FF.');
end
...
runDir=create_run_directory(fullfile(root,'data','ff'),"acceleration_"+string(job.id));

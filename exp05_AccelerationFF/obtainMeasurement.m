...
if method=="ilc_teacher" && isfield(teacherOptions,'allocation')
    trialResult.allocation = teacherOptions.allocation;
elseif startsWith(method,"allocation_")
    trialResult.allocation = allocationSettings;
end
...
try
    ...
    elseif startsWith(method,"allocation_")
        [measurement,trialResult.allocationLogFile] = capture_with_allocation( ...
            @(requestId)capture_tunable_trajectory(ModelName,r,f,Ts,runDir,requestId), ...
            allocationSettings,Ts,runDir);
    else
    ...
    if method == "ilc_teacher"
        ...
        assert(trialResult.teacher.converged, 'NikonMotor:ILCTeacherNotConverged', ...
            'ILC did not converge within %d trials. Inspect ilc_progress_teacher before continuing.', Ntrial);
    end
...
resultFile = finalize_experiment_result(runDir, 'result', trialResult, 'raw', ...
    "ilc_progress_teacher.mat");

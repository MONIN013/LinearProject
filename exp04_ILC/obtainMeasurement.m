...
if exist("ilcExtension", "var") && ~isempty(fieldnames(ilcExtension))
    assert(~(exist("velocitySweepEnabled", "var") && velocitySweepEnabled) && ...
        ~(exist("teacherSettings", "var") && ~isempty(teacherSettings)), ...
        'NikonMotor:ConflictingILCUpdate', ...
        'An experimental update requires the single-trajectory ILC path.');
end
...
elseif exist("teacherSettings", "var") && ~isempty(teacherSettings)
    ...
    if isfield(extension, "initialFeedforward")
        ...
        assert(numel(initial)==N && all(isfinite(initial)), ...
            'NikonMotor:InvalidInitialFF', 'Initial FF must be finite and match the reference.');
        history.f(:,1) = initial;
    end
...
function [history, completedTrials, measurement] = run_ilc_trials( ...
        ...
        Qsos, Qscale, MAX_INPUT, confirmEachTrial, runDir, velocityTag, teacherSettings, extension)
...
[history, completedTrials, measurement] = run_ilc_trial_sequence( ...
    ...
    MAX_INPUT, confirmEachTrial, dataDir, bufferCapacity, runDir, velocityTag, teacherSettings, extension);
end
...
for velocityIndex = 1:numel(vMaxList)
    ...
    resultFiles(velocityIndex) = string(finalize_experiment_result( ...
        ...
        fullfile('raw',velocityTag), sprintf('ilc_progress_%s.mat',velocityTag)));
...
setvars(model,fixed_feedback_coefficients(evalin('base','Kd'),Ts));
...
function [history, completedTrials, measurement] = run_ilc_trial_sequence( ...
        ...
        runDir, velocityTag, teacherSettings, extension)
if nargin<17, teacherSettings = []; end
if nargin<18, extension = struct(); end
isTeacher = ~isempty(teacherSettings);
useExtension = isfield(extension, 'update');
assert(~(isTeacher && useExtension), 'NikonMotor:ConflictingILCUpdate', ...
    'Teacher qualification and an experimental update cannot be combined.');
updateState = struct();
...
measurement = [];
initialState = read_tunable_idle_state(Ts);
sequenceStart = initialState.positionCount;
if isTeacher, sequenceStart = teacherSettings.startCount; end
...
for iteration = firstTrial:Ntrial
    ...
    if abs(history.pre{iteration}.positionCount-sequenceStart)>1000
        ...
        [~,history.rehoming{iteration}.homing] = home_to_start(sequenceStart);
        [dataDir,bufferCapacity] = prepare_external_mode(model,numel(r),Ts);
        connect_external_mode(model);
        history.pre{iteration} = read_tunable_idle_state(Ts);
        history.rehoming{iteration}.after = history.pre{iteration};
    end
    assert(abs(history.pre{iteration}.positionCount-sequenceStart)<=1000, ...
        'NikonMotor:StartPositionChanged', 'ILC starting position changed; sequence stopped.');
    if isTeacher && isfield(teacherSettings,'allocation')
        trialDir = fullfile(runDir,'raw',velocityTag,sprintf('trial_%03d',iteration));
        [measurement,history.allocationLogFiles{iteration}] = capture_with_allocation( ...
            @(requestId)capture_trial(model,r,f,Tend,dataDir,Ts,bufferCapacity,runDir,velocityTag,iteration,requestId), ...
            teacherSettings.allocation,Ts,trialDir);
    else
    ...
    assert(max(abs(history.u(:,iteration)))<=MAX_INPUT+1e-6, ...
        'NikonMotor:CurrentLimitExceeded','Recorded total current exceeds the target limit.');
    assert(max(abs(history.e(:,iteration)))<3e-3 && ...
        abs(history.y(end,iteration))<100e-6 && ...
        all(history.y_absolute(:,iteration)>=5.4054804 & history.y_absolute(:,iteration)<=6.6655810), ...
        'NikonMotor:TrackingFailed','ILC tracking, return position or travel range failed.');
    if isTeacher
        history.teacher = evaluate_ilc_teacher( ...
            history, iteration, teacherSettings.velocity, teacherSettings);
        completedTrials = iteration;
        save_experiment_result(runDir, sprintf("ilc_progress_%s",velocityTag), struct( ...
    ...
    if useExtension
        ...
        assert(numel(fNext)==numel(f) && all(isfinite(fNext)), ...
            'NikonMotor:InvalidILCUpdate', 'The experimental update must be finite and match the reference.');
    ...
    assert(all(isfinite(fNext)), ...
        'NikonMotor:InvalidILCUpdate','The next learned waveform must be finite.');
    if iteration < Ntrial
        history.f(:, iteration+1) = fNext;
    end
    if useExtension
        % exp08 keeps its final unexecuted candidate in updateInfo.nextFF.
        save_experiment_result(runDir, sprintf("ilc_progress_%s",velocityTag), struct( ...
            "history",history,"completedTrials",completedTrials,"Ts",Ts));
    end
...
function [measurement, trialDir, metadata] = capture_trial( ...
        model, r, f, Tend, dataDir, Ts, bufferCapacity, runDir, ...
        velocityTag, iteration, allocationRequestId)
if nargin<11, allocationRequestId = uint32(0); end
trialDir = fullfile(runDir,'raw',velocityTag,sprintf('trial_%03d',iteration));
...
runValues.p_servo = 0;
runValues.p_allocation_request_id = uint32(allocationRequestId);
...
assert(string(get_param(model,"ExtModeConnected"))=="on" && ...
    ...
    'NikonMotor:ExternalModeDisconnected','External Mode disconnected before the ILC trial.');
setvars(model, struct("p_active", 1));
pause(Tend + 1);

setvars(model, struct("p_active", 0, "p_servo", 0));
pause(0.1); % allow motor_config to propagate servo OFF before clearing the ID
...
if ~ismember(metadata.schema_name,["feedforward_v1","feedforward_axis_v2"])
